...
from pyramidkv.pyramidkv_utils import init_pyramidkv,init_snapkv,init_CAM,init_H2O,init_StreamingLLM,init_l2norm, init_adakv, init_headkv, init_kv_cluster
...
# ============================================================
def mistral_attn_forward_ThinK(
    self,
    hidden_states: torch.Tensor,
    attention_mask: Optional[torch.Tensor] = None,
    position_ids: Optional[torch.LongTensor] = None,
    ...
    **kwargs,
    ...
    if past_key_value is not None:
        ...
        if key_states.shape[-2] >= kv_seq_len:  # prefill
            ...
            if len(result) == 3:
                ...
                self._think_keep_dim_idx = keep_dim_idx
    ...
    if hasattr(self, '_think_keep_dim_idx') and self._think_keep_dim_idx is not None and attn_output.shape[-1] < self.head_dim:
        ...
        keep_idx_o = self._think_keep_dim_idx.unsqueeze(1).expand(-1, q_len, -1, -1)
...
# ============================================================
def mistral_attn_forward_MiniCache(
    self,
    hidden_states: torch.Tensor,
    attention_mask: Optional[torch.Tensor] = None,
    position_ids: Optional[torch.LongTensor] = None,
...
def mistral_attn_forward_Custom(
    self,
    hidden_states: torch.Tensor,
    attention_mask: Optional[torch.Tensor] = None,
    position_ids: Optional[torch.LongTensor] = None,
