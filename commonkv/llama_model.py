...
def llama_attn_forward_ThinK(
    ...
    if past_key_value is not None:
        ...
        if hasattr(self, "kv_seq_len"):
            kv_seq_len += self.kv_seq_len if self.kv_seq_len != 0 else past_key_value.get_usable_length(kv_seq_len, self.layer_idx)
        else:
            kv_seq_len += past_key_value.get_usable_length(kv_seq_len, self.layer_idx)
    ...
    if past_key_value is not None:
        cache_kwargs = {"sin": sin, "cos": cos, "cache_position": cache_position}
        if key_states.shape[-2] == kv_seq_len:  # prefill phase
            self.kv_seq_len = kv_seq_len
            result = self.kv_cluster.update_kv(key_states, query_states, value_states,
                                               attention_mask, self.num_key_value_groups)
            if len(result) == 3:
                key_states_c, value_states_c, keep_dim_idx = result
                self._think_keep_dim_idx = keep_dim_idx   # [bsz, heads, n_keep]
                past_key_value.update(key_states_c, value_states_c, self.layer_idx, cache_kwargs)
                keep_idx_q = keep_dim_idx.unsqueeze(2).expand(-1, -1, q_len, -1)
                query_states = query_states.gather(dim=-1, index=keep_idx_q)
                key_states   = key_states_c
                value_states = value_states_c
            else:
                key_states, value_states = result[0], result[1]
                past_key_value.update(key_states, value_states, self.layer_idx, cache_kwargs)
        else:
            self.kv_seq_len += q_len
            key_states, value_states = past_key_value.update(key_states, value_states, self.layer_idx, cache_kwargs)
            if hasattr(self, '_think_keep_dim_idx') and self._think_keep_dim_idx is not None:
                keep_idx_q = self._think_keep_dim_idx.unsqueeze(2).expand(-1, -1, q_len, -1)
                query_states = query_states.gather(dim=-1, index=keep_idx_q)
        past_key_value._seen_tokens = self.kv_seq_len

    head_dim_k = key_states.shape[-1]
    ...
    # attn_output: [bsz, heads, q_len, n_keep] → scatter back to full head_dim
    attn_output = attn_output.transpose(1, 2)  # [bsz, q_len, heads, n_keep]
    if hasattr(self, '_think_keep_dim_idx') and self._think_keep_dim_idx is not None and attn_output.shape[-1] < self.head_dim:
        full_out = torch.zeros(bsz, q_len, self.num_heads, self.head_dim,
                               dtype=attn_output.dtype, device=attn_output.device)
        keep_idx_o = self._think_keep_dim_idx.unsqueeze(1).expand(-1, q_len, -1, -1)  # [bsz, q_len, heads, n_keep]
        full_out.scatter_(dim=-1, index=keep_idx_o, src=attn_output)
        attn_output = full_out
...
def llama_sdpa_attn_forward_APKVC(
        self,
        hidden_states: torch.Tensor,
        attention_mask: Optional[torch.Tensor] = None,
        position_ids: Optional[torch.LongTensor] = None,
